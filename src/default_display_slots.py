...
from src.display_slots import DisplayContext, get_display_slot_registry
...
def render_header(context: DisplayContext, cli: "CLI") -> list[str]:
    ...
    return [
        ...
        f"Lines: {len(context.score.lines) if context.score else 0}"[: context.width - 1],
        "",
    ]
...
def render_score(context: DisplayContext, cli: "CLI") -> list[str]:
    ...
    for line_idx in range(start_line, end_line):
        ...
        line_text = cli._format_score_line(line) if hasattr(cli, "_format_score_line") else ""
...
def render_status(context: DisplayContext, cli: "CLI") -> list[str]:
    ...
    if context.player and context.total_lines > 0 and context.score:
        ...
        played_notes = sum(len(context.score.lines[i]) for i in range(context.current_line))
...
def render_config(context: DisplayContext, cli: "CLI") -> list[str]:
    ...
    lines.append(f"  Space Interval: {space_interval:.1f}x     [Shift+Up/Down] Adjust space")
    lines.append(
